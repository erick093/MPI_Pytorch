...
def main():
    ...
    if rank == 0:
        ...
        if utils.DEBUG:
            # val_sample, train_sample = train_test_split(train_df, test_size=0.03, stratify=train_df['category_id']).copy()  # at least n=64500 / 2,257,759
            # test_sample = val_sample.sample(utils.N_IMAGES, random_state=0).reset_index(drop=True).copy()
    ...
    # TODO: Add support for pretrained models from pytorch
    ...
    for epoch in range(utils.NUM_EPOCHS):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss.backward()
            LOGGER.info("_Epoch: {}, sharing gradients".format(epoch))
        ...
        if rank == 0: # FIXME: Move model evaluation outside the for loop, its lagging the other processes
            ...
            val_loss = 0
            running_corrects = 0
            #preds = np.zeros((len(test_sample)))
